...
if (project_id = ENV["ERRBIT_PROJECT_ID"]) && project_key = (ENV["ERRBIT_PROJECT_KEY"])
  Airbrake.configure do |c|
    ...
    c.error_host = ENV.fetch("ERRBIT_HOST", "")
  ...
  Airbrake.add_filter do |notice|
    if notice[:errors].any? { |error| error[:type] == "SignalException" && error[:message] =~ /SIGTERM|SIGINT/ }
      notice.ignore!
    end
  end
